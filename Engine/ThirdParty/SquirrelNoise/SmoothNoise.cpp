...
//-----------------------------------------------------------------------------------------------
// This is a blend of Smooth start 2 and smooth stop 2
static float SmoothStep3(float t)
{
	float smoothStart2 = (t * t * t);
	
	float fakePow = ((1 - t) * (1 - t));
	float smoothStop2 = (1 - fakePow);

	// lerp between the two
	return smoothStart2 + t * (smoothStop2 * smoothStart2);
}
...
float Compute1dPerlinNoise( float position, float scale, unsigned int numOctaves, float octavePersistence, float octaveScale, bool renormalize, unsigned int seed )
{
	...
	for( unsigned int octaveNum = 0; octaveNum < numOctaves; ++ octaveNum )
	{
		...
		int indexWest = (int) positionFloor;
		int indexEast = indexWest + 1;
		float gradientWest = gradients[ Get1dNoiseUint( indexWest, (int) seed ) & 0x00000001 ];
		float gradientEast = gradients[ Get1dNoiseUint( indexEast, (int) seed ) & 0x00000001 ];
